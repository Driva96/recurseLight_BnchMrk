...
```{r benchmarking with bench, eval=F}
args <- list(
  x = data_smooth[[1]][, c("X", "Y", "time", "TAG")],
  radius = 50,
  timeunits = "mins"
)
argsLM <- list(
...
# Light 
bnchL <- bench::mark(
  rlang::exec(recurseLight::getRecursions,!!!args)
)
#Light Multithreading
bnchLM <- bench::mark(
  rlang::exec(recurseLight::getRecursions,!!!argsLM),
  iterations = 1
)
#Olds implementation 
bnch <- bench::mark(
  rlang::exec(recurse::getRecursions,!!!args),
  iterations = 1
)
```

```{r bench all with bench, eval=F}
bnchA<-bench::mark(
  rlang::exec(recurseLight::getRecursions,!!!args),
  rlang::exec(recurseLight::getRecursions,!!!argsLM),
  rlang::exec(recurse::getRecursions,!!!args),
  iterations=5, check = F, memory = T
  
)
```

```{r benchmark with rbenchmark, eval=F}
testset <- data_smooth[[1]][, c("X", "Y", "time", "TAG")]
rbenchmark::benchmark(
  #recurseLight_Sing = recurseLight::getRecursions(testset,radius = 50, timeunits = "mins", ncores = 1),
  recurseLight_Multi = recurseLight::getRecursions(testset,radius = 50, timeunits = "mins", ncores = ncores),
  recurse_old = recurse::getRecursions(testset,radius = 50, timeunits = "mins"),
  order = "relative", replications = 5
)
```


```{r print results, eval=F}
bnchR<- list(c("recurse","recurseLight","recurseLight (OpenMP)"),c(as.character(bnch$mem_alloc), as.character(bnchL$mem_alloc),as.character(bnchLM$mem_alloc)),c(as.character(bnch$median),as.character(bnchL$median),as.character(bnchLM$median)))
writeLines(
  sprintf("Allocated memory %s version on dataset with %d rows: %s; Elapsed time: %s", bnchR[[1]], nrow(args[[1]]), bnchR[[2]],bnchR[[3]])
)
```

Check if Mulithreading approach yields same result. 
```{r is equal, eval=F}
all(
  isTRUE(all.equal(drop(rlang::exec(recurseLight::getRecursions,!!!args)), drop(rlang::exec(recurseLight::getRecursions,!!!argsM))))
)
```

```{r}
test <- data_smooth[[1]][, c("X", "Y", "time", "TAG")]
results <- recurseLight::getRecursions(
    x = test,
    radius = 50,
    timeunits = "mins",
    ncores = ncores 
...
data_residence <- lapply(data_smooth, function(dt) {
  ...
  dt_recurse <- recurseLight::getRecursions(
    x = dt[, c("X", "Y", "time", "TAG")],
    radius = 50,
    timeunits = "mins",
    ncores = ncores
  ...
  # keep everything left of the first true
  # first true is first time since last visit >5min --> defines stationary point 
...
data_patches <- lapply(data_residence, atl_res_patch,
  buffer_radius = 25,
  min_fixes = 3,
  summary_variable = c("night", "resTime"), #"distance_roost"),
...
In the following I plot the data directly in R 
```{r plotting sf features}
mp <- st_read("data/data_bat_residence_patches.gpkg")
```

```{r plot sf}
plot(mp['patch'],reset = F)
#plot(mp[3,2], col = 'grey', add = TRUE)
```
